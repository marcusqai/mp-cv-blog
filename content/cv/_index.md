---
title: "Curriculum Vitae"
description: "APAC Supplier Quality & Operations Leader with expertise in sustainable supplier management, manufacturing transfer, and quality transformation"
---

## Profile

**APAC Supplier Quality & Operations Leader**

📧 [mail@marcuspoon.eu.org](mailto:Marcus%20Poon%20%3Cmail@marcuspoon.eu.org%3E)

Over 20 years of management experience in MNC environments, driving supplier quality transformation and manufacturing relocation across the APAC region. Expertise in building sustainable supplier management systems from the ground up, establishing localized testing infrastructure, and elevating partner capabilities through systematic development programs.

---

## Core Competencies

### Supplier Quality Management
- Built sustainable supplier management systems from the ground up
- Established comprehensive supplier qualification, audit, and scorecard frameworks

### Manufacturing Transfer & Relocation
- Led manufacturing relocations from China to Thailand, Vietnam, and Cambodia
- Managed risk mitigation and quality continuity throughout transition phases

### Test Laboratory Development
- Built a testing center from scratch covering dimensional, chemical, mechanical, metallurgical, electronic, electrical, and reliability testing
- Designed custom fixtures and jigs aligned with ASTM standards

### Automation & Standardization
- Introduced automation processes to ensure test consistency and reproducibility
- Established standardized testing protocols across regional operations

### AI-Driven Data Analysis
- Integrated AI for data analysis and uncertainty research
- Validated cross-laboratory data reliability through round-robin testing

### Cross-Domain Collaboration & R&D Support
- Assisted suppliers in establishing witness labs for institutional certification
- Developed data-driven testing solutions for new product development and technical troubleshooting

### Team Leadership & Organization
- Built and managed quality teams across Mainland China, Hong Kong, Thailand, Taipei City, and Indonesia
- Developed regional quality organization structure in multinational environment

---

## Industry Focus

Premium outdoor electronic & electrical appliances, gas appliances — delivering supplier quality excellence, regulatory compliance, warranty cost reduction, and comprehensive testing lab management in MNC environments across APAC markets.

---

## Languages

| Language | Proficiency |
|----------|-------------|
| English | Native |
| Cantonese | Native |
| Mandarin | Native |

---

## References

Available upon request.
